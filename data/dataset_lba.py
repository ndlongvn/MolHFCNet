...
from data.PDBBind_utils import get_pocket_res, get_pdb_code, PocketSelect, find_files, identity_split, write_index_to_file


from rdkit import Chem
import numpy as np

def create_rdkit_mol(atom_features_list, position_list):
    """
    Create an RDKit molecule from atom features and positions.
    :param atom_features_list: List of atomic numbers or indices.
    :param position_list: List of 3D coordinates for each atom.
    ...
    for atomic_number in atom_features_list:
        if atomic_number == "unknown":
            # Handle unknown atoms (e.g., skip or use a placeholder)
            continue
        atom = Chem.Atom(int(atomic_number))  # Ensure atomic number is an integer
        mol.AddAtom(atom)

    # Add a conformer with the given positions
    conf = Chem.Conformer(len(position_list))
    for i, (x, y, z) in enumerate(position_list):
        conf.SetAtomPosition(i, (x, y, z))  # Set 3D coordinates for each atom
    ...
    def process(self):
        ...
        for data_id, (pdb_id, data) in enumerate(tqdm(structure_dict.items(), desc="creating pyg data")):
            ...
            for ia in range(len(merged_df)):
                ...
                y = merged_df['y'][ia]
                z = merged_df['z'][ia]
                atom_features_list.append(atomic_number)
                position_list.append([x, y, z])

            mol = create_rdkit_mol(atom_features_list, position_list)
            x_active, positions_active = atom_attr(mol)

            edge_index, _ = bond_attr(mol)
            edge_index = torch.LongTensor(edge_index).t()
            
            atom_features_list = torch.tensor(x_active, dtype=torch.float)
            position_list = torch.tensor(positions_active, dtype=torch.float)
